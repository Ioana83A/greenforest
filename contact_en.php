<?php
	...
	class Contact extends PageTemplate
	{
		public function execute()
		{
			...
			$this->templateEngine->load($data, 'contact_en.html');
		}
		
		private function getText()
		{
			return '<div style="overflow:hidden"><div class="category_outer" ><div class="category_inner"><h1>Contact S.C. <span id="company_name">GreenForest</span> S.R.L.</h1></div><div class="clear_box"></div></div><div class="clear_box"></div></div><br/><br/>
		...
		private function getContactForm(){
			...
			$profil = "Activity profile";
			$mesaj="Your message";
			if(isset($_POST['nume'])){
			
			    if($_POST['nume']  != 'Last name') $style_input['nume']= "color:#000000";
				if($_POST['prenume']  != 'First name') $style_input['prenume']= "color:#000000";
				if($_POST['email']  != 'E-mail address') $style_input['email']= "color:#000000";
				if($_POST['nr_tel']  != 'Phone') $style_input['nr_tel']= "color:#000000";
				if($_POST['nume_firma']  != 'Company name') $style_input['nume_firma']= "color:#000000";
				if($_POST['profil_activitate']  != 'Activity profile') $style_input['profil_activitate']= "color:#000000";
				if($_POST['mesaj']  != 'Your message') $style_input['mesaj']= "color:#000000";
				...
				if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
				                           $send_status = "E-mail address is not valid!";
										   $style_send_status = 'style="color:#CC0000"';
										  }
			    else if(!$this->validPhone($_POST['nr_tel']))
				{
					 $send_status = "Phone number is not valid!";
					 $style_send_status = 'style="color:#CC0000"';
				}
				else if(($_POST['mesaj']=="") ||($_POST['mesaj']=="Your message")) 
				{
					 $send_status = "You haven't introduced a message!";
					 $style_send_status = 'style="color:#CC0000"';
				}
				else
				{
					...
					else
					{
						...
						$profil = "Activity profile";
						$mesaj="Your message";
						$send_status = "The message has been sent!";
			...
			$form='<div id="contact_right" class="col-sm-7 col-xs-12"><div class="contact_form" style="width:510px;float:right;">
			      <form id="contact_form" action="en/Contact#send" method="post">
				   ...
				   <div id="send_btn" style="margin-top:10px;float:right;"><button type="button" class="btn btn-default" id="send">Send</button></div>
				   <div class="clear_box"></div>
				   <div class="send_status_outer" '.$style_send_status.'><div class="send_status_inner">'.$send_status.'</div></div>
				   <div class="clear_box"></div>
				   </form>
				   <a name="down"></a> 
				   </div></div>';
		...
		private function getCanonicalURL(){
			return 'http://www.greenforest.ro/en/Contact';
		}
