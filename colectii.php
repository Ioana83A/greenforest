<?php
	...
	class Colectii extends PageTemplate
	{
		public function execute()
		{
			...
			{
				$href_next_collection = 'Colectia-mobilier-de-birou-'.$collection->getNextCollection();
			}
			else
			{
				$href_next_collection = 'Colectia-mobilier-de-birou-ErgoPlus';
			}

			if($collection->getPrevCollection() != NULL) 
			{
				$href_prev_collection = 'Colectia-mobilier-de-birou-'.$collection->getPrevCollection();
			}
			else
			{
				$href_prev_collection = 'Colectia-mobilier-de-birou-Movi';
			}
			...
			$data = array(
				...
				'{Link_en}'=>$collection->getLinkEn(),
				'{Canonical_URL}'=>$collection->getCanonicalURL(),
				'{OG_Image}'=>$collection->getOGImage()
				);
			
			$this->templateEngine->load($data, 'colectii.html');
		}
