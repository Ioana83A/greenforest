...
class ChairsEn{
    ...
    public function displayPageContent()
    {
		...
		else
		{
            ...
            foreach ($product_ids as $product_id) {
                ...
                $this->products[] = array(
				    'name' => $row1->name_en,
					'short_name' => $row1->short_name_en,
                    'seo_name' => $row1->seo_name_en,
                    'image' => $row1->image,
                    'alt_image' => $row1->alt_image_en,
      ...
      if(!$this->productId)
	  {
          return '<div style="overflow:hidden;margin-top:47px;margin-bottom:25px" ><div class="category_outer"><div class="category_inner" ><span id="category_title">'.$this->getCategoryName().'</span></div> <div class="clear_box"></div></div></div>'.$products_display;
	  }
	  else
	  {
            ...
            $row = $this->query->result->fetch_object();
                ...
                $row1 = $this->query->result->fetch_object();
                    ...
                    if ($row1->arms_chairs_img)
                        ...
                        $product_info .= ' <div class="product_detailed_info"><div id="variant_description"><h1>' . $row1->product_name_en . '</h1><p>' . $row1->size_info . '</p>
									  <div id="desc" style="border-bottom:none;padding-left:0">' . $row1->description_en ;
									  
									 
						if($row1->optional_chairs_en)
                               $product_info .= '<div id="colors_title" style="margin-bottom:3px">Optional:</div>'.$row1->optional_chairs_en;
						 
					   
									 
						if($row1->variants_en)
                               $product_info .= '<div id="colors_title" style="margin-bottom:3px">Variants:</div>'.$row1->variants_en;  
									
									  $product_info .='';
						 if($row1->available_online_en)			  
									 $product_info .= '<div id="colors_title">Order online:</div>
									  <div style="padding:5px 0px 5px 0px">'.$row1->available_online_en.'</div>';
                        ...
                        $product_info .= '<div class="clear_box"></div></div>
									  ...
									  <div id="variant_images"><a href="images/products/' . $row1->image_big . '" class="image-popup" title="'.$row1->alt_image_en.'"><div style="width:417px;height:200px;margin-bottom:10px;background-image:url(\'images/products/image-bg-1.jpg\');background-repeat:repeat;text-align:center;box-shadow: 3px 3px 3px 0px rgba(0, 0, 0, 0.35);"><img src="images/products/' . $row1->image_big . '"  style="width:417px;height:200px" alt="' . $row1->alt_image_en . '" /></div></a>';
    ...
    {
        if ($this->categoryName != "Chairs") {
            $this->query->execute("SELECT `description_en` FROM `collection_categories` WHERE `category_id`=" . $this->categoryId . " AND  `collection_id`=" . $this->collectionId);
            $row  = $this->query->result->fetch_object();
