...
class Category{
    ...
    public function __construct($query, $categ_seo_name, $active_subcategory_seo_name, $collection_seo_name, $product_seo_name)
    {
        ...
        if ($this->categoryParrentId != NULL) {
            $this->query->execute("SELECT * FROM `categories` WHERE `id` = " . $this->categoryParrentId);
            $row                          = $this->query->result->fetch_object();
            $this->categoryParrentSeoName = $row->seo_name;
        } else {
            $this->categoryParrentSeoName = NULL;
        }
    ...
    public function displayCategoryProducts()
    {
        ...
        if ($this->collectionId == NULL) {
            ...
            if ($num_rows > 0) {
                while ($row = $this->query->result->fetch_object()) {
                    $this->products[] = array(
                        ...
                        'image_small' => $row->image_big,
                        'alt_image_small' => $row->alt_image_big
                    );
                }
            }
        } else {
            ...
            foreach ($product_ids as $product_id) {
                ...
                $this->products[] = array(
                    ...
                    'image_small' => $row1->image_big,
                    'alt_image_small' => $row1->alt_image_big
                );
            }
        }
        ...
        for ($i = $offset; $i < count($this->products); $i++) {
            ...
            $products_display .= '<div><li><a href="' . $href . '/p=' . $this->products[$i]['seo_name'] . '&id=' . $new_offset . '" class="project_image"><img src="images/products/' . $this->products[$i]['image_small'] . '" alt="' . $this->products[$i]['alt_image_small'] . '"/></a>
 ...
 vezi detalii</a></p></div></li></div>';
    ...
    public function displayCategoryProductsSmallScreen()
    {
        ...
        for ($i = 0; $i < count($this->products); $i++) {
            $offset = floor($i / 3) * 3;
            $products_display .= '<li><a href="' . $href . '/p=' . $this->products[$i]['seo_name'] . '&id=' . $offset . '" class="product_image"><img src="images/products/' . $this->products[$i]['image_small'] . '" alt="' . $this->products[$i]['alt_image_small'] . '"/></a>
			<div class="carousel-caption" style="padding-top:10px;padding-bottom:0">         
    ...
    public function displaySubcategories()
    {
        ...
        if ($this->productId == NULL && $this->activeSubcategoryName != NUll) {
            ...
            if ($this->categoryId != SCAUNE_ID && $this->categoryId != SCAUNE_ERGONOMICE_ID) {
    ...
    public function generateCollections()
    {
        ...
        if ($this->categoryId == 1 || $this->categoryId == 3 || $this->categoryId == 6 || $this->categoryId == 2) {
            
            $options        = '<div class="clear_box"></div><div class="filter"><div id="ddtopmenubar1" class="mattblackmenu">
					 <ul>
						<li ><a href="#" rel="ddsubmenu11" id="collections">Colectie</a></li>
					 </ul>
					</div>
            ...
            $this->query->execute("SELECT `collections`.* FROM `collections` JOIN `collection_categories` ON `collections`.`id` = `collection_categories`.`collection_id` WHERE `collection_categories`.`category_id`=" . $this->categoryId);
            if ($this->query->result->num_rows) {
                while ($row1 = $this->query->result->fetch_object()) {
                    
                    $found = false;
                    for ($j = 0; $j < (count($collections_id) - 1); $j++)
                        if ($collections_id[$j] == $row1->id)
                            $found = true;
                    
                    if (!$found) {
                        $collections_id[] = $row1->id;
                    }
                }
            ...
            for ($i = 0; $i < (count($collections_id) - 1); $i++) {
                for ($j = 0; $j < (count($collections_id) - ($i + 1)); $j++) { {
                        if ($collections_id[$j] > $collections_id[$j + 1]) {
                            $aux                    = $collections_id[$j];
                            $collections_id[$j]     = $collections_id[$j + 1];
                            $collections_id[$j + 1] = $aux;
                        }
                    }
                }
            }
            ...
            foreach ($collections_id as $collection_id) {
                $this->query->execute("SELECT * FROM `collections` WHERE `id`=" . $collection_id);
                $row = $this->query->result->fetch_object();
                $options .= '<li><a href="' . $this->categorySeoName . '/colectia-' . $row->seo_name . '#scroll">' . $row->name . '</a></li>';
            }
            
            $options .= '</ul></div>';
        } else if (($this->categoryId == SCAUNE_ID) || ($this->categoryId == SCAUNE_ERGONOMICE_ID)) {
            $options = '<div class="clear_box"></div><div class="filter"><div id="ddtopmenubar1" class="mattblackmenu">
					 <ul>
						<li ><a href="#" rel="ddsubmenu11" id="collections">Categorie</a></li>
					 </ul>
					</div>
			<script type="text/javascript">
				ddlevelsmenu.setup("ddtopmenubar1", "topbar");
			...
			<ul id="ddsubmenu11" class="ddsubmenustyle">';
            $this->query->execute("SELECT * FROM `categories` WHERE parrent_id =" . SCAUNE_ID);
            while ($row = $this->query->result->fetch_object()) {
                $options .= '<li><a href="Scaune/' . $row->seo_name . '#scroll">' . $row->name . '</a></li>';
            }
            $options .= '</ul></div>';
        } else
            $options = '';
        ...
        if ($this->productId != NULL) {
            ...
            if (!isset($_REQUEST['variant_id'])) {
                ...
                if ($num_rows) {
                    ...
                    while ($row1 = $this->query->result->fetch_object()) {
                        ...
                        if ($num_rows == 1)
                            $id = "";
                        if ($nb_var < $num_rows) {
                            $variants .= '<a class="product_variant" href="' . $href . '" ' . $id . '>' . $row1->short_name . '<input type="hidden" class="variant_image" value="images/products/' . $row1->image_big . '"/></a>';
                        } else {
                            $variants .= '<a class="product_variant" href="' . $href . '" ' . $id . '>' . $row1->short_name . '<input type="hidden" class="variant_image" value="images/products/' . $row1->image_big . '"/></a>';
                        }
                    }
                ...
                $product_info .= ' <div class="product_info"><div id="product_title"><div id="product_title_outer"><div id ="product_title_inner"><h1 style="white-space:nowrap;">' . $row->name . '</h1></div></div></div>
								 ...
								 <div id="product_big_image"><img src="images/products/' . $row->image_big . '" style="box-shadow: 2px 2px 2px 0px rgba(0, 0, 0, 0.35);" alt="' . $row->alt_image_big . '"/></div>
                ...
                if (($row->category_id == DULAPURI_CAT_ID || $row->category_id == CASETIERE_CAT_ID) && ($row->collection_id != COLECTIA_ELITE_ID) && ($row->collection_id != COLECTIA_INNO_ID)) {
                    if ($row->collection_id == COLECTIA_ERGO_ID) {
                        $product_info .= ' <div class="product_detailed_info"><div id="variant_description"><h1>' . $row1->product_name . '</h1><p>(' . $row1->long_name . ')</p>
                    ...
                    if ($row1->variant_id == X_PANDER_ID) {
                        ...
                        $product_info .= ' <div class="product_detailed_info" style="height:700px"><div id="variant_description"><h1>' . $row1->product_name . '</h1><p>' . $row1->long_name . '</p>
                        ...
                        $product_info .= ' <div class="product_detailed_info" style="height:750px"><div id="variant_description"><h1>' . $row1->product_name . '</h1><p>' . $row1->long_name . '</p>
									  <div id="desc" style="border-bottom:none;padding-left:0">' . $row1->description . '
									  <div id="colors_title">Bra&#355;e:</div>
									  <div style="float:left;width:330px;padding-left:20px;padding-top:3px">' . $row1->arms_chairs_text . '</div>
									  <div style="float:left;">' . $img_arms . '</div>
                        ...
                        $product_info .= ' <div class="product_detailed_info" style="height:850px"><div id="variant_description"><h1>' . $row1->product_name . '</h1><p>' . $row1->long_name . '</p>
									  <div id="desc" style="border-bottom:none;padding-left:0">' . $row1->description . '
									  <div id="colors_title">Bra&#355;e:</div>
									  <div style="float:left;width:330px;padding-left:20px;padding-top:3px">' . $row1->arms_chairs_text . '</div>
									  <div style="float:left;">' . $img_arms . '</div>
                        ...
                        $product_info .= ' <div class="product_detailed_info" style="height:650px"><div id="variant_description"><h1>' . $row1->product_name . '</h1><p>' . $row1->long_name . '</p>
									  <div id="desc" style="border-bottom:none;padding-left:0">' . $row1->description . '
									  <div id="colors_title">Bra&#355;e:</div>
									  <div style="float:left;width:330px;padding-left:20px;padding-top:3px">' . $row1->arms_chairs_text . '</div>
									  <div style="float:left;">' . $img_arms . '</div>
                    ...
                    if ($row1->variant_id == 1200 || $row1->variant_id == 1210) {
						...
						if ($row1->product_id == 500 || $row1->product_id == 501 || $row1->product_id == 503 || $row1->product_id == 504 || $row1->product_id == 505 || $row1->product_id == 506 || $row1->product_id == 507 || $row1->product_id == 508 || $row1->product_id == 510 || $row1->product_id == 511 || $row1->product_id == 512) {
						 	$detailed_info_height = '750px'; 
						 
						}
						else
						{
							$detailed_info_height = '550px';
						}
                        
                        $product_info .= ' <div class="product_detailed_info" style="height:'.$detailed_info_height.'"><div id="variant_description"><h1>' . $row1->product_name . '</h1><p>' . $row1->long_name . '</p>
									  <div id="desc" style="border-bottom:none;padding-left:0">' . $row1->description . '
									  <div id="colors_title">Optional:</div>
				    ...
				    if($row->collection_id == COLECTIA_MOVI_ID)
					{
						...
						$product_info .= ' <div class="product_detailed_info movi" style="height:840px"><div id="variant_description" style="width:550px"><h1>' . $row1->product_name . '</h1><p>(' . $row1->long_name . ')</p>
									  <div id="desc">' . $row1->description . '</div>
									  <div id="colors_title">Culori disponibile:</div>
